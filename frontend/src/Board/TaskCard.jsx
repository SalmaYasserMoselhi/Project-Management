...
const TaskCard = ({
  ...
  useEffect(() => {
    if (initialMembers && initialMembers.length > 0) {
      console.log(
        `[TaskCard.jsx] ✅ Using optimized members data for card ${id}:`,
        initialMembers
      );
      ...
      if (membersCache.has(id)) {
        ...
        console.log(
          `[TaskCard.jsx] Using cached members for card ${id}:`,
          cachedMembers
        );
      ...
      console.log(
        `[TaskCard.jsx] ⚠️ Fallback: Fetching members for card ${id}`
      );
  ...
  const handleCardSaved = (originalListId, newListId) => {
    ...
    // The cardUpdated event will handle list changes, so we just call onCardUpdate for general updates
    if (onCardUpdate) {
      onCardUpdate();
    }
  };
    ...
    try {
      ...
      if (response.ok && result.status === "success") {
        ...
        if (response.status === 403) {
          ...
          toast.error("Unable to archive card. Please try again");
        }
      }
    } catch (error) {
  ...
  const handleDragStart = (e) => {
    ...
    e.dataTransfer.setData("isCompleted", isCompleted.toString());
    e.dataTransfer.setData("priority", priority || "Medium");
    e.dataTransfer.setData("labels", JSON.stringify(labels));
    ...
    try {
      ...
      if (err.response?.status === 403) {
        const errorMessage =
          ...
          "You don't have permission to mark this card as completed";
