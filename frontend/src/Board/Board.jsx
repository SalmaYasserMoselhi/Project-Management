...
import { Check, ChevronDown, ChevronRight, X, ArrowUp, ArrowDown, User } from 'lucide-react'
...
const Board = ({ workspaceId, boardId, restoredLists }) => {
  ...
  const applySort = () => {
    ...
    toast.success(`Sort applied: ${tempSort === "date" ? "Due Date" : "Priority"} (${tempSortDirection === "asc" ? "Ascending" : "Descending"})`)
    setIsSortOpen(false)
  }
  ...
  const handleFilterByPriority = async (priority) => {
    try {
      ...
      if (res.data.data.cards && res.data.data.cards.length) {
        cardsByList = columns.map((col) => {
          console.log(
            `[Board.jsx] Checking list ${col.id || col._id} (${col.name}) with col.id:`,
            col.id,
            "col._id:",
            col._id,
          )
  ...
  const applyFilters = async () => {
    ...
    if (tempFilters.assignedMember) {
      const result = await handleFilterByAssignedMember(tempFilters.assignedMember)
      if (result) {
        appliedCount++
        const memberName = tempFilters.assignedMember === "me" 
          ? "Me" 
    ...
    switch (category) {
      ...
      case "dueDate":
        return tempFilters.dueDate ? 
          (tempFilters.dueDate === "all" ? "All dates" : tempFilters.dueDate) : 
          "Any date"
      default:
        return "Any"
    }
                    ...
                    <div className="absolute z-10 mt-2 right-0 bg-white border border-gray-200 rounded-lg shadow-lg w-56">
                      ...
                      <div className="p-1">
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <div className="w-4">
                              {tempSort === "date" && <Check className="h-4 w-4 text-[#4D2D61]" />}
                            </div>
                      ...
                      <div className="border-b border-gray-50">
                        <div className="p-3">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              <span className="text-sm text-gray-500">{getFilterSummary("assignedMember")}</span>
                          ...
                          <div className="bg-gray-50 p-3 border-t border-gray-50">
                            <div
                              className={`flex items-center gap-2 p-2 rounded-md cursor-pointer hover:bg-white ${
                                tempFilters.assignedMember === "me" ? "bg-white" : ""
                              }`}
                              onClick={() => setTempFilters(prev => ({...prev, assignedMember: "me"}))}
                            >
                              <div className="w-4">
                                {tempFilters.assignedMember === "me" && <Check className="h-4 w-4 text-[#4D2D61]" />}
                              </div>
                            ...
                            {boardMembers.map((member) => (
                              <div
                                ...
                                onClick={() => setTempFilters(prev => ({...prev, assignedMember: member.id}))}
                              >
                                <div className="w-4">
                        ...
                        {activeFilterSubmenu === "dueDate" && (
                          <div className="bg-gray-50 p-3 border-t border-gray-50">
                            <div
                              ...
                              onClick={() => setTempFilters(prev => ({...prev, dueDate: "overdue"}))}
                            >
                              <div className="w-4">
                            ...
                            <div
                              ...
                              onClick={() => setTempFilters(prev => ({...prev, dueDate: "dueSoon"}))}
                            >
                              <div className="w-4">
                            ...
                            <div
                              ...
                              onClick={() => setTempFilters(prev => ({...prev, dueDate: "dueThisWeek"}))}
                            >
                              <div className="w-4">
                            ...
                            <div
                              ...
                              onClick={() => setTempFilters(prev => ({...prev, dueDate: "all"}))}
                            >
                              <div className="w-4">
                                {tempFilters.dueDate === "all" && <Check className="h-4 w-4 text-[#4D2D61]" />}
                              </div>
