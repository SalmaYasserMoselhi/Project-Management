...
const Board = ({
  ...
  const onListRestored = useCallback(
    (data) => {
      ...
      console.log("[Board.jsx] Restored cards:", restoredCards);

      setColumns((prevColumns) => {
        const updatedColumns = allLists.map((list) => {
          if (list._id === restoredList._id) {
            // For the restored list, include the cards from the backend response
            return {
              ...restoredList,
              id: restoredList._id,
              cards: restoredCards || [], // Use the cards returned by the backend
            };
          }
        ...
        console.log(
          "[Board.jsx] Updated columns after list restore:",
          updatedColumns
        );
  ...
  useEffect(() => {
    const handleCardUpdatedEvent = (event) => {
      ...
      setColumns((prevColumns) => {
        return prevColumns.map((col) => {
          ...
          if (cardIndex !== -1 && cardIndex !== undefined) {
            ...
            if (updatedData.listId && updatedData.listId !== col.id) {
              // Remove the card from this list since it was moved to a different list
              const filteredCards = col.cards.filter(
                (c) => (c.id || c._id) !== cardId
              );
              console.log(
                `[Board.jsx] Removed card ${cardId} from column ${col.id} after move to ${updatedData.listId}`
              );
      ...
      // Card updates are handled by the Column components via events
      // No need to dispatch additional refresh events here
    };
      ...
      const fetchLists = async () => {
        try {
          ...
          if (error.response?.status === 403) {
            ...
            toast.error(
              "Unable to load board content. Please refresh the page"
            );
          }
        } finally {
          setLoading(false);
  ...
  const handleListDrop = async (e, targetCol) => {
    ...
    try {
      ...
      if (err.response?.status === 403) {
        toast.error("You don't have permission to reorder lists");
      } else if (err.response?.status === 404) {
        toast.error("List not found");
      } else {
        toast.error("Unable to reorder lists. Please try again");
      }
  ...
  const updateColumnsWithCards = (
    ...
    // Only dispatch refresh events for columns that actually have new data
    // This prevents unnecessary API calls to columns that haven't changed
    updatedColumns.forEach((col) => {
      const listData = cardsByList.find(
        (list) => list.listId === col.id || list.listId === col._id
      );

      // Only dispatch refresh event if this column has data or needs to be updated
      if (listData) {
  ...
  const fetchSortedCards = async (
    ...
    try {
      ...
      console.error("[Board.jsx] Error sorting cards:", error);
      toast.error("Unable to sort cards. Please try again");
    }
  };
  ...
  const handleFilterByPriority = async (priority) => {
    try {
      ...
      toast.error("Unable to filter cards by priority. Please try again");
      return false;
    }
  };
  ...
  const handleFilterByDate = async (dueDateFilter) => {
    try {
      ...
      toast.error("Unable to filter cards by date. Please try again");
      return false;
    }
  };
  ...
  const handleFilterByAssignedMember = async (memberId) => {
    try {
      ...
      toast.error(
        "Unable to filter cards by assigned member. Please try again"
      );
      return false;
    }
  };
  ...
  const applyFilters = async () => {
    ...
    try {
      ...
      console.error("[Board.jsx] Error applying filters:", error);
      toast.error("Unable to apply filters. Please try again");
      success = false;
    }
  ...
  const clearAllFilters = async () => {
    try {
      ...
      toast.error("Unable to clear filters. Please try again");
      return false;
    }
  };
  ...
  return (
    <div className="pb-6 min-h-screen flex flex-col item-center overflow-y-auto w-full">
      <style>{styles}</style>
      <div className="border-2 border-[#C7C7C7] p-4 rounded-xl h-full flex flex-col w-full">
        <div className="flex flex-col lg:flex-row sm:flex-row justify-between items-center lg:items-start sm:items-start mb-6 w-full">
          <div className="flex items-center gap-6 mb-4 lg:mb-0 sm:mb-0">
        ...
        {view === "board" && (
          <div className="flex-1 overflow-y-auto pb-4 w-full">
            <div
              className={`flex flex-col lg:flex-row gap-4 min-w-0 h-full lg:overflow-x-auto w-full`}
            >
              {columns.map((col, index) => (
                ...
                >
                  ...
                  <Column
                    id={col.id || col._id}
                    title={col.name}
                    className="w-full lg:min-w-[300px] h-full"
                    boardId={boardId}
                    allLists={columns}
