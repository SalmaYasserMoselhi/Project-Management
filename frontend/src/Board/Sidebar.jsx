      ...
      <nav className="space-y-2">
        {sidebarItems.map((item) => (
          <div
            key={item.title}
            className={`flex items-center justify-between px-3 py-2.5 rounded-lg cursor-pointer transition-all duration-200 ${
              activeItem === item.title ? "bg-[#4D2D61] text-white" : "text-gray-900 hover:bg-purple-50"
            } ${isSidebarOpen ? "w-full" : "w-14 justify-center"}`}
            onClick={(e) => {
              e.stopPropagation();
              handleItemClick(item.title);
          ...
          >
            <div className="flex items-center gap-3">
              <img 
                src={item.icon} 
                alt={`${item.title} icon`} 
                className={`h-6 w-6 ${activeItem === item.title ? "filter brightness-0 invert" : "filter brightness-0"}`}
              />
              ...
              <div className="flex items-center gap-2 ml-2">
                {item.hasAdd && (
                  <img
                    src='src/assets/add.png'
                    alt="Add"
                    className={`h-4 w-4 ${activeItem === item.title ? "filter brightness-0 invert" : "filter brightness-0"}`}
                  />
                )}
                {item.hasDropdown && (
                  <img
                    src="src/assets/drop-down.png"
                    alt="Dropdown"
                    className={`h-5 w-5 ${activeItem === item.title ? "filter brightness-0 invert" : "filter brightness-0"}`}
                  />
                )}
              </div>
            )}
